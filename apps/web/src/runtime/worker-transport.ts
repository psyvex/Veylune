...
interface WorkerPort { postMessage(message: unknown, transfer?: Transferable[]): void; addEventListener(type: "message", listener: (event: MessageEvent) => void): void; addEventListener(type: "error", listener: (event: ErrorEvent) => void): void; removeEventListener(type: "message", listener: (event: MessageEvent) => void): void; removeEventListener(type: "error", listener: (event: ErrorEvent) => void): void; terminate?(): void; }
type WorkerMessage<TOutput> = { readonly type: "progress"; readonly status: JobStatus } | { readonly type: "completed"; readonly status: JobStatus; readonly output: TOutput } | { readonly type: "failed"; readonly status: JobStatus } | { readonly type: "cancelled"; readonly status: JobStatus };
...
export class WorkerJobTransport<TInput, TOutput> implements JobTransport<TInput, TOutput> {
  ...
  private readonly active = new Map<string, "running" | "cancelling">();
  ...
  private onWorkerError(event: ErrorEvent): void { for (const id of [...this.active.keys()]) { this.active.delete(id); const status: JobStatus = { id, state: "failed", progress: { completed: 0, total: 0 }, error: { code: "WORKER_ERROR", message: event.message || "Worker execution failed.", retryable: true } satisfies JobError }; this.emit({ type: "failed", status }); } }
  private emit(event: JobEvent<TOutput>): void { for (const listener of this.listeners) listener(event); }
}
function isValidId(id: unknown): id is string { return typeof id === "string" && id.length > 0 && id.length <= 128; }
function isWorkerMessage<TOutput>(value: unknown): value is WorkerMessage<TOutput> { if (!value || typeof value !== "object") return false; const message = value as Record<string, unknown>; if (message.type !== "progress" && message.type !== "completed" && message.type !== "failed" && message.type !== "cancelled") return false; const status = message.status; if (!status || typeof status !== "object") return false; const candidate = status as Record<string, unknown>; const progress = candidate.progress; if (!isValidId(candidate.id) || typeof candidate.state !== "string" || !progress || typeof progress !== "object") return false; const p = progress as Record<string, unknown>; return Number.isFinite(p.completed) && Number.isFinite(p.total) && p.completed >= 0 && p.total >= 0 && p.completed <= p.total; }
