...
export class ReconstructionOptimizationBridge {
  ...
  notifyKeyframeInserted(): number { this.version += 1; this.setState("pending"); this.scheduler.request(this.version); return this.version; }
  ...
  private setState(phase: OptimizationPhase): void { this.phase = phase; optionsState(this.state, undefined); }
}
function optionsState(state: OptimizationBridgeState, callback?: (state: OptimizationBridgeState) => void): void { callback?.(state); }
