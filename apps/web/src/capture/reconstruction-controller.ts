...
export interface ReconstructionControllerProgress { readonly jobId: string; readonly completed: number; readonly total: number; readonly fraction: number; }
...
export class ReconstructionController {
  ...
  async optimize(maxIterations = 5): Promise<ReconstructionControllerResult> {
    ...
    const snapshot = this.store.snapshot(); const expectedMapVersion = snapshot.map.version; const expectedPoseVersion = snapshot.poses.version;
    ...
    try {
      const outputPromise = new Promise<ReconstructionWorkerOutput>((resolve, reject) => {
        this.unsubscribe = this.transport.subscribe((event) => {
          ...
          if (event.type === "progress") { const total = Math.max(0, event.status.progress.total); const completed = Math.min(total, Math.max(0, event.status.progress.completed)); this.options.onProgress?.({ jobId, completed, total, fraction: total > 0 ? completed / total : 0 }); }
          else if (event.type === "completed") resolve(event.output);
