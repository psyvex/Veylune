...
export function optimizeBundle(input: BundleProblem, options: Partial<BundleOptimizerOptions> = {}): BundleOptimizationResult {
  ...
  let problem = cloneProblem(input); let damping = config.initialDamping; let currentCost = bundleCost(problem);
  ...
  for (let iteration = 0; iteration < config.maxIterations; iteration += 1) {
    ...
    const blocks = assembleBundleBlocks(linearization, damping); const solved = solveBundleSchurBlocks(blocks, damping);
    if (solved.status !== "solved") { damping = Math.min(config.maxDamping, damping * 10); if (damping >= config.maxDamping) return { status: "rejected", iterations: iteration + 1, initialCost, finalCost: currentCost, problem }; continue; }
    ...
    if (!Number.isFinite(candidateCost) || candidateCost >= currentCost) { damping = Math.min(config.maxDamping, damping * 4); continue; }
    const improvement = currentCost - candidateCost; const stepNorm = Math.max(norm(solved.cameraStep), norm(solved.landmarkStep)); problem = candidate; currentCost = candidateCost; damping = Math.max(config.minDamping, damping / 3);
