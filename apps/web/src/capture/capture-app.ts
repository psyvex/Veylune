...
export function mountCaptureApp(root: HTMLElement, capabilities: CapabilityProfile, telemetry: Partial<CaptureTelemetry> = {}): CaptureApp {
  let camera: CameraSession | undefined; let scan: LiveScanSession | undefined; let processor: LiveReconstructionProcessor | undefined; let running = false;
  ...
  const startCamera = async (): Promise<void> => {
    ...
    try {
      ...
      const extractor = new LocalVisionExtractor({ maxKeypoints: 384 }); const matcher = new BruteForceDescriptorMatcher(0.8);
      const poseEstimator = new LocalPoseEstimator({ intrinsics, extractor, matcher });
      processor = new LiveReconstructionProcessor({ intrinsics, extractor, matcher, poseEstimator, onSession: (session) => { state.tracking = processor?.trackingState === "recovering" ? "recovering" : "tracking"; state.keyframes = session.map.keyframes.length; state.reconstructionVersion = session.map.version; updateTelemetry(); } });
    ...
    } catch (error) { status.textContent = error instanceof Error ? error.message : "Unable to start camera."; start.disabled = false; }
  };
  const stopCamera = (): void => { scan?.stop(); processor?.dispose(); scan = undefined; processor = undefined; camera = undefined; running = false; state.tracking = "idle"; state.keyframes = 0; state.reconstructionVersion = 0; updateTelemetry(); snapshot.disabled = true; stop.disabled = true; start.disabled = false; status.textContent = "Camera stopped"; };
