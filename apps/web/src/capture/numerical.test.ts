...
describe("capture numerical foundations", () => {
  ...
  it("reduces a valid damped Schur system", () => {
    const result = buildSchurReducedSystem({
      camera: { rows: 2, columns: 2, values: new Float64Array([4, 0, 0, 4]) },
      cameraLandmark: { rows: 2, columns: 2, values: new Float64Array([1, 0, 0, 1]) },
      landmark: { rows: 2, columns: 2, values: new Float64Array([2, 0, 0, 2]) },
      cameraGradient: new Float64Array([1, 2]),
      landmarkGradient: new Float64Array([1, 1]),
    }, 0.1);
    expect(result).toBeDefined();
