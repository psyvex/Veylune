...
function fromBundleResult(session: ReconstructionSessionSnapshot, problem: BundleProblem): ReconstructionSessionSnapshot { const cameras = new Map(problem.cameras.map((camera) => [camera.id, camera.pose])); const landmarks = new Map(problem.landmarks.map((landmark) => [landmark.id, landmark])); return { ...session, map: { ...session.map, version: session.map.version + 1, landmarks: session.map.landmarks.map((landmark) => landmarks.get(landmark.id) ?? landmark), keyframes: session.map.keyframes.map((keyframe) => ({ ...keyframe, pose: cameras.get(keyframe.id) ?? keyframe.pose })) }, poses: { ...session.poses, version: session.poses.version + 1, poses: session.poses.poses.map((pose) => ({ ...pose, pose: cameras.get(pose.id) ?? pose.pose })) }, createdAtMs: Date.now() }; }
