...
export class LiveReconstructionProcessor implements ScanProcessor {
  ...
  async process(frame: ScanFrame): Promise<boolean> { const features = await this.extractor.extract(frame); this.lastKeypoints = features.keypoints; this.lastFrameWidth = frame.image.width; this.lastFrameHeight = frame.image.height; if (features.keypoints.length < 4) { this.lastRejectReason = `low-features(${features.keypoints.length})`; this.handleFailure(); return false; } const pose = await this.poseEstimator.estimate(frame, this.previousPose); if (!pose) { this.lastRejectReason = "pose-failed"; this.handleFailure(); return false; } const captureFrame: CaptureFrame = { id: `frame:${this.frameIndex}`, frameIndex: this.frameIndex++, timestampMs: frame.timestampMs, features }; const result = this.previousFeatures ? this.pipeline.process(captureFrame, this.matcher, pose) : this.pipeline.initialize(captureFrame, pose); if (!result.accepted) { this.lastRejectReason = `${result.reason}(inliers=${result.inliers},conf=${result.confidence.toFixed(2)})`; this.handleFailure(); return false; } this.lastRejectReason = ""; this.recovery.accept(); this.previousFeatures = features; this.previousPose = pose; if (result.session) this.onSession?.(result.session); if (result.keyframeInserted) this.optimizationBridge?.notifyKeyframeInserted(); return true; }
