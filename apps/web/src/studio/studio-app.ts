...
import { applyTheme, currentTheme, initTheme, MOON_ICON, storeTheme, SUN_ICON, toggleTheme } from "../design/theme-preference";
...
export function mountStudioApp(root: HTMLElement, capabilities: CapabilityProfile): { dispose(): void } {
  ...
  let routeToken = 0;
  // Shared with the marketing page (design/theme-preference.ts): one stored
  ...
  root.addEventListener("click", (event) => {
    ...
    if (target.closest('[data-action="theme"]')) { toggleTheme(); return; }
