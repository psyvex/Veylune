...
pub enum ExecutionBackend { WebGpu, Wasm, Native }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum QualityTier { Preview, Balanced, High, Maximum }
...
impl CapabilityProfile {
    pub fn preferred_backend(&self) -> ExecutionBackend {
        if self.webgpu { ExecutionBackend::WebGpu } else { ExecutionBackend::Wasm }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ResourceClass { Small, Medium, Large }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum JobState { Queued, Running, Cancelling, Cancelled, Completed, Failed }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RuntimeError { InvalidTransition, ResourceLimitExceeded, Cancelled }
...
mod tests {
    ...
    fn falls_back_to_wasm_without_gpu() {
        let profile = CapabilityProfile { webgpu: false, wasm_simd: true, wasm_threads: false, quality: QualityTier::Balanced };
        assert_eq!(profile.preferred_backend(), ExecutionBackend::Wasm);
    }

    #[test]
    fn progress_is_bounded() {
        let status = JobStatus { state: JobState::Running, completed_steps: 9, total_steps: 3 };
